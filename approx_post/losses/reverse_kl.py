...
def reverse_kl(approx, joint, method='elbo', use_reparameterisation=True, verbose=False):
    ...
    def loss_and_grad(params, x, num_samples):
        ...
        phi = Jaxtainer(approx.phi(params, x))
        if method == 'elbo':
...
def elbo_reparameterisation(phi, x, approx, joint, num_samples):
    ...
    epsilon_samples = approx.sample_base(num_samples) # shape = (num_samples, theta_dim)
    theta_samples = approx.transform(epsilon_samples, phi) # shape = (num_samples, theta_dim)

    approx_lp = approx.logpdf(theta_samples, phi) # shape = (num_batch, num_samples) 
    approx_del_1 = approx.logpdf_del_1(theta_samples, phi) # shape = (num_batch, num_samples, dim_theta) 
    transform_del_phi = approx.transform_del_2(epsilon_samples, phi) # shape = (num_batch, num_samples, theta_dim, *phi.shape)
    joint_lp = joint.logpdf(theta_samples, x) # shape = (num_batch, num_shape)
    joint_del_theta = joint.logpdf_del_1(theta_samples, x) # shape = (num_batch, num_shape, theta_dim)
    ...
    grad = -1*np.mean(grad_samples, axis=1) # shape = (num_batch, *phi.shape)
...
def elbo_controlvariates(phi, x, approx, joint, num_samples):
    ...
    theta_samples = approxdist.sample(num_samples, phi) # shape = (num_batch, num_samples, theta_dim)

    approx_lp = approx.logpdf(theta_samples, phi) # shape = (num_batch, num_samples)
    joint_lp = joint.logpdf(theta_samples, x) # shape = (num_batch, num_samples)
    ...
    grad_samples = np.einsum("ab,ab...->ab...", loss_samples, approx_del_phi) # shape = (num_batch, num_samples, *phi.shape)

    control_variate = approx_del_phi
    num_batch = loss_samples.shape[0]
    loss = -1*apply_cv(loss_samples[:,:,None], control_variate, num_batch, num_samples) # shape = (num_batch,)
    grad = -1*apply_cv(grad_samples, control_variate, num_batch, num_samples) # shape = (num_batch, *phi.shape)
...
def selbo_reparameterisation(phi, x, approx, joint, num_samples):
    ...
    approx_lp = approx.logpdf_components["lp"](theta_samples, phi) # approx_lp.shape = (num_mixture, num_batch, num_samples) 
    approx_del_1 = approx.logpdf_del_1_components["lp_del_1"](theta_samples, phi) 
    # approx_del_1.shape = (num_mixture, num_batch, num_samples, dim_theta) 
    transform_del_phi = approx.transform_del_2["transform_del_2"](epsilon_samples, phi) 
